...
<style>
    ...
    .search-unified { background: #ffffff; border-radius: 12px; padding: 6px; box-shadow: 0 10px 20px rgba(0, 0, 0, 0.1); display: flex; align-items: center; width: 100%; }
    .search-group { display: flex; align-items: center; flex: 1; padding: 5px 15px; border-right: 1px solid var(--tvlk-border); }
    ...
    .btn-search-unified { background-color: var(--tvlk-orange); color: white; font-weight: 700; font-size: 1rem; border-radius: 8px; padding: 12px 25px; border: none; transition: 0.3s; }
    .btn-search-unified:hover { background-color: var(--tvlk-orange-hover); }

    /* --- SIDEBAR FILTER --- */
    .filter-sidebar { background: white; border-radius: var(--radius-md); border: 1px solid var(--tvlk-border); padding: 20px; position: sticky; top: 20px; }
    ...
    /* --- SORTING BAR --- */
    .sorting-bar { display: flex; justify-content: space-between; align-items: center; background: white; padding: 15px 20px; border-radius: var(--radius-md); border: 1px solid var(--tvlk-border); margin-bottom: 20px; }
    ...
    /* --- TVLK TOUR CARD --- */
    .tvlk-card { background: white; border-radius: var(--radius-md); box-shadow: 0 2px 8px rgba(3, 18, 26, 0.05); border: 1px solid var(--tvlk-border); overflow: hidden; transition: 0.2s; height: 100%; display: flex; flex-direction: column; position: relative; }
    ...
    /* BADGE SAO ĐÁNH GIÁ (Cập nhật màu sắc) */
    .rating-badge { position: absolute; bottom: 10px; left: 10px; background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(4px); padding: 4px 10px; border-radius: 20px; font-size: 0.8rem; font-weight: 700; color: var(--tvlk-text); display: flex; align-items: center; gap: 4px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
    
    /* BADGE BÁN CHẠY NHẤT (MỚI THÊM) */
    .best-seller-badge {
            ...
            <div class="row g-4" id="tour-list-container">
                <?php if (!empty($tours)): ?>
                    <?php foreach ($tours as $row): ?>
                        <div class="col-md-6 col-lg-4 tour-card-wrapper" data-price="<?= $row['price'] ?>" data-duration="<?= $row['duration'] ?? 0 ?>">
                            <div class="tvlk-card">
                                <div class="card-img-box">
                                    ...
                                    // Xử lý dữ liệu động (Fallback an toàn nếu Controller chưa lấy được data)
                                    $rating = !empty($row['avg_rating']) ? round($row['avg_rating'], 1) : 5.0; 
                                    $reviewCount = !empty($row['review_count']) ? $row['review_count'] : 0;
                                    $totalBookings = !empty($row['total_bookings']) ? $row['total_bookings'] : 0;
                                    ?>
                                    
                                    <img src="<?= $imgSrc ?>" alt="<?= htmlspecialchars($row['tour_name']); ?>">
                                    
                                    <?php if ($totalBookings > 2): ?>
                                        <div class="best-seller-badge"><i class="bi bi-fire"></i> Bán chạy</div>
                                    <?php endif; ?>
                                ...
                                <div class="tvlk-card-body">
                                    ...
                                    <h5 class="tour-title" title="<?= htmlspecialchars($row['tour_name']); ?>"><?= htmlspecialchars($row['tour_name']); ?></h5>
                                    ...
                                    <ul class="tour-info-list">
                                        ...
                                        <?php if ($totalBookings > 0): ?>
                                            <li><i class="bi bi-bag-check-fill text-success"></i> Đã bán <?= $totalBookings ?> vé</li>
                                        <?php endif; ?>
                                    </ul>
...
<script>
    document.querySelectorAll('.filter-price, .filter-duration').forEach(checkbox => { checkbox.addEventListener('change', runFilters); });
    function runFilters() {
        ...
        tours.forEach(tour => {
            ...
            let priceMatch = false; let durationMatch = false;

            if (selectedPrices.length === 0) { priceMatch = true; } else {
                for (let p of selectedPrices) {
                    let [min, max] = p.split('-');
                    if (tourPrice >= parseInt(min) && tourPrice <= parseInt(max)) { priceMatch = true; break; }
                }
            }

            if (selectedDurations.length === 0) { durationMatch = true; } else {
                for (let d of selectedDurations) {
                    let [min, max] = d.split('-');
                    if (tourDuration >= parseInt(min) && tourDuration <= parseInt(max)) { durationMatch = true; break; }
                }
            }

            if (priceMatch && durationMatch) {
                tour.classList.remove('d-none'); visibleCount++;
            } else {
        ...
        if (visibleCount === 0 && tours.length > 0) { noResultMsg.classList.remove('d-none'); } else { noResultMsg.classList.add('d-none'); }
    }
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow" style="border-radius: 20px;">
            <div class="modal-header bg-light border-0" style="border-radius: 20px 20px 0 0;">
                <h5 class="modal-title fw-bold" style="color: var(--tvlk-blue);"><i class="bi bi-calendar-check me-2"></i>Chọn lịch trình</h5>
            ...
            <div class="modal-body p-4">
                ...
                <form action="index.php" method="GET">
                    ...
                    <select name="departure_id" id="departureSelect" class="form-select form-select-lg mb-4" style="border-radius: 12px; font-size: 1rem;" required>
                        <option value="">Đang tải dữ liệu...</option>
                    </select>

                    <?php if (isset($_SESSION['user'])): ?>
                        <button type="submit" class="btn w-100 py-3 fw-bold shadow-sm text-white" style="border-radius: 12px; font-size: 1.1rem; background: var(--tvlk-orange);">
                            Xác nhận đặt tour
                        </button>
                    <?php else: ?>
                        <a href="index.php?action=login" class="btn w-100 py-3 fw-bold shadow-sm text-white" style="border-radius: 12px; font-size: 1.1rem; background: var(--tvlk-blue);">
    ...
    document.querySelectorAll('.btn-book-now').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            fetch(`index.php?action=getDepartures&tour_id=${tourId}`)
                .then(res => { if (!res.ok) throw new Error('Phản hồi từ máy chủ không hợp lệ'); return res.json(); })
                .then(data => {
                    let html = '';
                    if (!data || data.length === 0) {
                        ...
                        data.forEach(d => {
                            ...
                            html += `<option value="${d.departure_id}" ${isDisabled}>${dateFormatted} (${seatText})</option>`;
                        });
                    }
                    select.innerHTML = html;
                })
                .catch(error => { console.error('Lỗi:', error); select.innerHTML = '<option value="" disabled>Lỗi kết nối. Vui lòng thử lại!</option>'; });
